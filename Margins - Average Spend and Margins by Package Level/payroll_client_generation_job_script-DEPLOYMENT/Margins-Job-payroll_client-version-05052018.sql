...
01/05/2018 Fredy - Corrected comparison between bill_date and Max payroll month. Changed to datetime instead of char compare.
05/05/2018 Fredy - Eliminated last 6 months condition 
*/
    DECLARE  @Implemented_Payroll_Date DATETIME = '2017-01-01';
		...
		FROM (
				...
				FROM (
					...
					outer apply (
					 ...
					 where a.bill_date >= @Implemented_Payroll_Date 
					and c.name <> 'None'
					and bill_date_end is not null
					and a.bill_date <= 
					(SELECT DATEADD(s,-1,DATEADD(mm, DATEDIFF(m,0,
							 max(cast(left([month],2)+'/01/'+substring([month],4,4) as datetime)) --> MAX MONTH ON PAYROLL
						  )+1,0)) --> GET LAST DAY OF MAX MONTH ON PAYROLL
