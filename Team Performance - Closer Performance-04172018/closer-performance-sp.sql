...
--Prefers Date	429
---None-	1
--CREATE PROCEDURE DBO.get_closers_oe_performance
--AS
DECLARE @Closer varchar(50)= 'Alberto Orozco' , @MobileApp varchar(5) = 'NO', @LastTreeMonth DATETIME = DATEADD(MONTH, DATEDIFF(MONTH, 0, GETDATE()) - 2, 0)
...
-->Profile Tone (only considering avg of 1st 2 selections)
...
FROM (
    ...
    SELECT  a.Closer, Stages.Name
		  ...
		  ,case when Stages.CloseDate=1 then datediff(day,min(c.[Closed Time]), [Closing Date]) else 0 end [DaysToCloseDate_CRM]
		  ,sum(case when Stages.CloseDate=1 and c.[Closed Time]< [Closing Date] then 1 else 0 end) [OESToCloseDate_CRM]
		  ,sum(case when Stages.CloseDate=1 and nr.[Closed Time]< [Closing Date] then 1 else 0 end) [NRToCloseDate_CRM]
		  ,ISNULL(count(distinct a.[Closing Date]),0) OppCount
		  ,case when Stages.CloseDate=1 then 1 else 0 end CloseDateOpp
		  ,case 
		  when Stages.GotPhoneNumber=1 and acc.[Date / Number Preference] LIKE '%phone%' then 1 
		  when Stages.GotDate=1 and acc.[Date / Number Preference] LIKE '%Date%' then 1
		  when Stages.CloseDate=1 AND (acc.[Date / Number Preference] LIKE '%No%Preference%' OR acc.[Date / Number Preference] IS NULL) then 1
		  else 0 end [AchievingClients_Pref]
        FROM [VIDA].[dbo].[opportunities] a
             LEFT JOIN [TRACK].dbo.users b ON ISNULL(a.Closer,'Missing') = b.name
             INNER JOIN VIDA.dbo.sites d ON REPLACE(REPLACE(a.[Site], CHAR(13), ''), CHAR(10), '') = d.origin
		   inner join VIDA.dbo.accounts acc on acc.[Clients Id] = a.[Clients Id]
        inner JOIN
        (
	   ...
	   INNER JOIN @tmp_Stages Stages ON Stages.Name = a.Stage
        WHERE 
		  --a.Stage IN('Got Phone Number'
			 ...
			 --, 'IC Deleted Profile'
			 --, 'Closed Lost'
			 --, 'Negative Response IE - SEND OE') AND
		  d.[Site] <> 'eHarmony'
            AND LTRIM(RTRIM(a.[Opportunities Name])) NOT LIKE 'SALES%'
		  ...
		  --and a.[Opportunities Name] ='Bill Vivino-Phoebe'
		  --and acc.[Clients Name]  = 'Bill Vivino'
		  group by a.Closer, acc.[Date / Number Preference], a.[Opportunities Name],a.[Closing Date], Stages.Name, Stages.CloseDate, Stages.GotPhoneNumber, Stages.GotDate
) t1 group by Closer
