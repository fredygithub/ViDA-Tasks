    ...
    (
	   ...
	   where a.closer = 'Scott Valdez'
        ...
        (
		  ...
		  --select distinct [Type] FROM [VIDA].[dbo].[tasks] 
		  --select top 50 * from [VIDA].[dbo].[tasks] 
		  --select * from VIDA.dbo.sites
		  --select [Opportunities Name], count(*) FROM [VIDA].[dbo].[opportunities] where [Opportunities Name] like '%sales%' group by [Opportunities Name] where closer like '%missin%' --147638
		  select sum(case when CHARINDEX(CHAR(13), [site])>0 then 1 else 0 end), 
				sum(case when CHARINDEX(CHAR(10), [site])>0 then 1 else 0 end)
				from [VIDA].[dbo].[opportunities]

				select [Opportunities Name] from opportunities where [Opportunities Name] LIKE '%SALES' --146838  146781 group by LTRIM(RTRIM([Opportunities Name])) --203501
				LTRIM(RTRIM(a.[Opportunities Name])) NOT LIKE 'SALES%'
